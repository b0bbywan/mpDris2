...
| `bridge.py` | `MpdMprisBridge` — MPD connect/reconnect, D-Bus export, MPRIS callbacks, idle-driven `refresh()` (incl. the queue → TrackList sync: `playlistinfo` re-fetched only when status's `playlist` version moves, diffed into TrackAdded/TrackRemoved or one TrackListReplaced) |
| `mpd_client.py` | `mpd.asyncio.MPDClient` wrapper: connect-with-backoff + capability probe |
| `mpris.py` | `dbus_fast.ServiceInterface` classes: `MediaPlayer2` (root) + `MediaPlayer2Player` + `MediaPlayer2TrackList` (the MPD queue; track ids are `/org/mpris/MediaPlayer2/Track/<songid>`) + `MediaPlayer2Playlists` (MPD stored playlists; ids are `/org/mpris/MediaPlayer2/Playlist/<hex-escaped name>`, `ActivePlaylist` follows status `lastloadedplaylist`, MPD >= 0.24). Full MPRIS 2.2 surface. |
